...
router.patch('/users/me',auth,async (req,res)=>{
    ...
    const allowedUpdateKeys = ['name','age','password']
    ...
    try {
        updateKeys.forEach((field)=> req.user[field] = req.body[field])
        await req.user.save()
        console.log(req)
...
const upload = multer({
    ...
    fileFilter(req,file,cb) {
        if(!file.originalname.match(/\.(jpeg|jpg|png)$/)) {
            return cb(new Error("incorrect format only images accepted"))
        }
        cb(undefined,true)
    }
})
